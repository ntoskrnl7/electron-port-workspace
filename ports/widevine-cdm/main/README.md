...
scripts/build-dev-electron-npm.sh --target main
```
...
```bash
scripts/port-bundle.sh apply widevine-cdm --target main --src-root /path/to/src
scripts/port-bundle.sh undo widevine-cdm --target main --src-root /path/to/src
```
